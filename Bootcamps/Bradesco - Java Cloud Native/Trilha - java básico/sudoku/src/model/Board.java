...
public class Board {
    ...
    public GameStatusEnum getStatus(){
        ...
        return spaces.stream().flatMap(Collection::stream).anyMatch(s -> Objects.isNull(s.getActual()))
         ? GameStatusEnum.INCOMPLETE : GameStatusEnum.COMPLETE;
    }
    public boolean hasErrors(){
        if (getStatus().equals(GameStatusEnum.NON_STARTED)) {
            return false;
        }
    ...
    public boolean clearSpace(int col, int row){
        var space = spaces.get(col).get(row);
    ...
    public boolean gameIsFinished(){
        return !hasErrors() && getStatus().equals(GameStatusEnum.COMPLETE) ? true : false ;
    }
}
